...
let UserDetail = async (req, res, next) => {
  try {
        if(req.params.userId==='me'){
          let user=await authService.findUser({_id:mongoose.Types.ObjectId(req.decoded._id)})
            if(user.length>0){
         ...
         return res.status(200).json({
          ...
          if(req.decoded.isAdmin===true){
            let user=await authService.findUser({_id:mongoose.Types.ObjectId(req.params.userId)})
            if(user.length>0){
            user=JSON.parse(JSON.stringify(user[0]))
           user.imagePath = authService.userImage(user.imagePath)
           delete user.password
